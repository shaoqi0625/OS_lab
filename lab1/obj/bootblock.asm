...
# start address should be 0:7c00, in real mode, the beginning address of the running bootloader 屏蔽中断，设置串地址增长方向，设置一些重要的数据段寄存器(DS,ES,SS)
.globl start
start:
...
seta20.2:
    ...
    movl %eax, %cr0  #将cr0的第0位置（PE）为1
    7c2a:	0f 22 c0             	mov    %eax,%cr0

    # Jump to next instruction, but in 32-bit code segment.
    # Switches processor into 32-bit mode.
    ljmp $PROT_MODE_CSEG, $protcseg #通过一个长跳转来更新CS寄存器的基地址
    7c2d:	ea                   	.byte 0xea
...
protcseg:
    # Set up the protected-mode data segment registers 是设置段寄存器并建立堆栈 初始化各个段寄存器并建立堆栈
